...
public class FriendRequestService {
    ...
    public void sendFriendRequest(BlogUser sender, BlogUser receiver) {
        if (!blogUserService.areFriends(sender, receiver) && !alreadySent(sender, receiver) && !alreadyReceived(sender, receiver)) {
    ...
    public boolean alreadySent(BlogUser sender, BlogUser receiver) {
        return friendRequestRepository.findBySenderAndReceiverAndStatus(sender, receiver, FriendRequestStatus.PENDING).isPresent();
    }

    public boolean alreadyReceived(BlogUser sender, BlogUser receiver) {
        return friendRequestRepository.findBySenderAndReceiverAndStatus(receiver, sender, FriendRequestStatus.PENDING).isPresent();
    }
    ...
    public Optional<FriendRequest> findBySenderAndReceiverAndStatus(BlogUser sender, BlogUser receiver, FriendRequestStatus status) {
